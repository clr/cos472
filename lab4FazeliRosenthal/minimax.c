/*****************************************************************************
 ...
 Casey Rosenthal
  ...
  This currently does 1 and 2, and we wrote below how we would go about 
...
3. staticEvaluator
  We would create an algorithm, that for every board, checked every column, row,
and diagonal to see if it had only one type of letter, either x or o.  We would
take the sum of those cases for x, and subtract the sum of those cases for o.
The answer would be the staticEvaluator score.
...
4. gameOver function is cheating, and just counting the number of moves
  ...
  We would write a function to see if all the moves are taken, and return true.
If there are still spaces open, we check each row, column, and diagonal
to see if either x or o occupies all three spaces, and if so return true.
